...
class RestBaseController
{
    ...
    {
        $token = $this->request->header('ml-Token');
        $deviceType = $this->request->header('ml-Device-Type');
        $type = $this->request->header('ml-type');
        if (empty($deviceType)) {
            return;
        }
        ...
        $user = Db::name('user_token')
            ->alias('a')
            ->field('b.*')
            ->where(['token' => $token, 'device_type' => $deviceType, 'type' => $type])
            ->join('Store b', 'a.user_id = b.id')
            ->find();
        if (!empty($user)) {
            $this->user = $user;
            $this->userId = $user['id'];
